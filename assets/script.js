...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  function buildCargoQuoteRecord(payload){
    return {
      ...
      weight_lb: Number(payload.weight || 0),
  ...
  if(cargoForm){
    ...
    function calculatePrice(){
      ...
      const note = '';
      const price = rate * usedWeight;
      return {price, usedWeight: Number(usedWeight.toFixed(2)), chargeable: Number(chargeable.toFixed(2)), rate, note};
    }
    ...
    function buildCargoQuotePayload(price){
      return {
    ...
    function buildCargoQuoteSaveKey(price){
      return JSON.stringify(buildCargoQuotePayload(price));
    }

    async function ensureCargoQuoteSaved(price){
      const saveKey = buildCargoQuoteSaveKey(price);
      if(lastSavedCargoQuote && lastSavedCargoQuote.key === saveKey){
        return lastSavedCargoQuote.data;
      }

      const data = await saveCargoQuote(buildCargoQuotePayload(price));
    ...
    function renderCargoCalculation(res, tone = 'info'){
      ...
      const breakdown = isEs ? `Peso: ${res.chargeable} lb · Peso usado: ${res.usedWeight} lb · Tarifa: $${res.rate}/lb` : `Weight: ${res.chargeable} lb · Used weight: ${res.usedWeight} lb · Rate: $${res.rate}/lb`;
    ...
    async function saveCurrentCargoQuote({ resetAfterSave }){
      ...
      if(res.chargeable <= 0){
        setCargoResultMessage(isEs ? 'Primero calcula con un peso válido.' : 'Calculate first with a valid weight.', 'error');
        feedbackEl?.classList.remove('ready');
        return;
      }
      ...
      try{
        const data = await ensureCargoQuoteSaved(res.price);
    ...
    [nameEl, emailEl, phoneEl, pickupEl, deliveryCityEl, branchEl, weightEl, typeEl].forEach((field) => {
    ...
    function openCargoWhatsApp(){
      ...
      msgLines.push((isEs ? 'Peso (lb)' : 'Weight (lb)') + ': ' + getVal(weightEl));
      if(lastCalc){
        msgLines.push('');
        ...
        msgLines.push((isEs ? 'Peso usado' : 'Used weight') + `: ${lastCalc.usedWeight} lb`);
        msgLines.push((isEs ? 'Tarifa' : 'Rate') + `: $${lastCalc.rate}/lb`);
      }
